...
class Image_processing:
    ...
    def import_images(self,folder):
        ...
        for image in a[1:]:    
            ...
            if 'numpy' in str(type(img[0][0])): #checks whether image is rgba or gray
                img = Image_processing.rgb2gray(img)
            images.append(img)
        return(images)
    ...
    def find_dust(self,threshold):
        ...
        dust_positions=[]
        if type(self.backgrounds) == list:
            bgsubtracted_image = self.images[self.activeimage]-self.backgrounds[self.activeimage]
        else:
            bgsubtracted_image = self.images[self.activeimage]-self.backgrounds
    ...
    def iterate_frames(self):
        for i in range(len(self.images)):
            ...
            [positions, bgsub_image] = self.find_dust(34)
            current_frame["pixels"]=self.collect_dust(positions)
            current_frame = self.characterise_dust(current_frame["pixels"])
            self.dust_every_frame[self.activeimage] = current_frame
